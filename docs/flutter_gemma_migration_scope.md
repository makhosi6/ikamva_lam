...
**Product invariant (entire app, all releases):** **On-device AI only.** There is **no server-side LLM inference** and **no hybrid or fallback** that sends prompts or activations to a remote model. **Model weights** ship **inside the app bundle** as a **`pubspec.yaml`** asset (e.g. **`.litertlm`**); **`flutter_gemma`** installs from **`fromAsset`** into plugin storage on first prepare. There is **no** HTTP download of weights. Network use for **non–AI-core** needs (e.g. optional sync) remains separate and documented elsewhere.
...
- **Bundled model delivery:** Production code installs via **`FlutterGemma.installModel`…`fromAsset`** using the path declared in **`model_asset_manifest.dart`** / **`pubspec.yaml`**. Weights persist in the plugin sandbox after copy; **`getActiveModel`** re-opens them between launches, with **re-install from asset** if open fails.
...
- Else on **Android / iOS** → **`FlutterGemmaLlmEngine`** (bundled asset → `FlutterGemma.installModel`…`fromAsset`, then `getActiveModel`)
- Else (e.g. desktop `flutter test` host) → **`StubLlmEngine`**

**Historical note:** an earlier **`ProcessLlmEngine`** + **`native/`** + GGUF / `llama-cli` path existed for hackathon velocity; it has been **removed** in favour of **`flutter_gemma`** and **bundled** **`.litertlm`** (or supported mobile formats per the plugin matrix).

**Desired state:** Run on-device inference through **`flutter_gemma`’s** supported model formats, installing into the plugin’s expected location via **`fromAsset`** at first prepare (and on recovery), then the **Modern API** (`FlutterGemma.getActiveModel`, `createSession`, streaming where applicable).
...
2. **Initialization:** Call `FlutterGemma.initialize(...)` early in `main.dart` (no HuggingFace token — weights are not downloaded from HF in production).

3. **New engine implementation:** e.g. `FlutterGemmaLlmEngine implements LlmEngine` (and optionally `StreamingLlmCapability`):

   - **`ensureLoaded`:** Try **`getActiveModel`**; on failure, **`installModel`…`fromAsset`** with the bundled path, then open again. Progress may be forwarded to UI callbacks.
...
4. **Engine selection in `LlmService`:** **Production builds** use **`FlutterGemmaLlmEngine`** on mobile when the bundled asset is listed in the app manifest. A **`StubLlmEngine`** remains for **automated tests and CI** where the plugin is not used. **`ProcessLlmEngine`** and **`native/`** are **removed**.

5. **Model policy:** Choose a **default bundled artifact** (family, size) aligned with **APK/IPA size**, RAM, and quality. Document **store listing / privacy** (large binary in the app bundle).
...
|------|--------|----------------------|
| Weights | GGUF + `llama-cli` | **`.litertlm`** (or supported mobile format) **bundled** in APK/IPA; **`fromAsset`** install; **persisted** by plugin |
...
1. **Default model:** **Gemma 4** class **`.litertlm`** (see `learner_app/lib/llm/gemma_model_config.dart`, **`model_asset_manifest.dart`**, and **`OBTAINING_MODELS.txt`**).
2. **Delivery:** Declare the weight file under **`pubspec.yaml`** **`assets`**; **no** compile-time download URL for weights.
...
- **No model / bad install:** If the **bundled asset** is missing from the manifest, the install fails, or on-disk weights are corrupt (e.g. user cleared app data), show a **blocking** error on the hub or in Settings warm-up / retry—**no** silent stub for child-facing flows on mobile.
- **Offline / airplane mode:** Core **inference must work** without network after prepare (weights are local).
...
- **Model provenance:** Obtain **`.litertlm`** (or supported bundles) from official / trusted channels; verify checksums where upstream publishes them; commit or CI-fetch the binary per team policy — document in **`OBTAINING_MODELS.txt`**.
...
- **On-device inference disclosure:** App Store / Play privacy forms and in-app **About** or onboarding should state that **all AI / LLM inference runs on device**, that **model weights ship inside the app** (first prepare copies them into plugin storage), and that **prompts and model outputs are not sent to your servers for inference** (align wording with any optional non-LLM network features).
...
- **Model management:** Optional “clear model cache” / purge plugin install for support; **first-run** **`fromAsset`** install is the normal path for weights.
...
- On a **reference device**, after **one successful `fromAsset` install**, real **`LlmService.generate`** flows work **only** through `flutter_gemma`; subsequent launches use on-disk weights unless recovery re-installs from the bundled asset.
...
- **Degraded modes** documented and manually smoke-tested: **airplane mode** after prepare, corrupt/missing on-disk model (recovery from asset), gate failure—no silent wrong content for child-facing flows.
...
- **No remote inference:** Code review confirms **no** calls to third-party or first-party **LLM inference** HTTP APIs; **no** HTTP fetch of model weights; `LlmService` resolves only to **on-device** engines or **test stubs**.
...
- **Rolling implementation notes:** root [`CHANGELOG.md`](../CHANGELOG.md) (*Unreleased*) and [`README.md`](../README.md) (Models; VS Code, `.env`, and CI) list concrete behaviour (bundled weights, hub warm-up, purge on corrupt archives, debug panel, timeouts, workflows) kept in sync with this scope.
