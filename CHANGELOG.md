...
- **Model delivery (no APK weights):** Gemma **`.task`** / **`.litertlm`** files are **not** bundled under `assets/`. Production mobile builds download with **`package:http`** using compile-time **`IKAMVA_MODEL_DOWNLOAD_URL`** (and optional **`IKAMVA_HF_TOKEN`**), then **`FlutterGemma.installModel(…).fromFile`** registers the cached file. See `learner_app/assets/models/OBTAINING_MODELS.txt`, repo-root **`.env.example`**, and README “Models”.
- **Persistence & recovery:** Weights are downloaded with `package:http` into a **fixed app support path** (`ikamva_gemma/ikamva_ondevice_model.bin` + URL meta JSON), then **`FlutterGemma.installModel(…).fromFile`** registers them (stable path makes “already downloaded?” checks reliable). **`getActiveModel`** is preferred on load. **`FlutterGemmaLlmEngine.ensureLoaded`** tries open → purge stale ids + delete cache → re-download if needed (same when **`LlmService.generate` / `tryOpenGenerateStream` / `ensureReady`** runs). **`LlmService.ensureReady`** timeout increased to **600s** for large downloads.
