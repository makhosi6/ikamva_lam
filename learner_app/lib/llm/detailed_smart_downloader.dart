...
String _taskStatusDetailForUser(TaskStatusUpdate update) {
  final b = StringBuffer('${update.status}');
...
class DetailedSmartDownloader {
  ...
  /// Last error from [FileDownloader.resume] (shown if all retries exhaust).
  static Object? _lastResumeFailure;

  /// UI / logs: automatic retry vs fatal (single-flight downloads only).
  static void Function(String line)? onDiagnostic;

  static void _notifyDiag(String line) => onDiagnostic?.call(line);
  ...
  static Future<void> _ensureConfigured(bool? foreground) async {
    ...
    if (foreground == true) {
      ...
      debugPrint('📲 DetailedSmartDownloader: Configured for ALWAYS foreground');
    } else if (foreground == false) {
      // Never foreground
      await downloader.configure(
        androidConfig: [(Config.runInForeground, Config.never)],
      );
      debugPrint('📲 DetailedSmartDownloader: Configured for NEVER foreground');
    } else {
      ...
      debugPrint(
          '📲 DetailedSmartDownloader: Configured for AUTO foreground (>${_foregroundThresholdMB}MB)');
    }
    ...
    if (cancelToken != null) {
      cancellationListener = cancelToken.whenCancelled.asStream().listen((_) async {
        ...
        if (!progress.isClosed) {
          _notifyDiag('[Fatal] Cancel requested — not retrying.');
          progress.addError(
  ...
  static Future<void> _downloadWithSmartRetry({
    ...
    if (currentAttempt == 1) {
      _lastResumeFailure = null;
    } else {
      _notifyDiag(
        '[Retry] Starting automatic transfer attempt $currentAttempt of $maxRetries.',
      );
    }
    ...
    try {
        ...
        listener = _getUpdatesStream().listen((update) async {
          ...
          if (update is TaskProgressUpdate) {
            ...
            if (update.status == TaskStatus.complete) {
              ...
              if (!progress.isClosed) {
                _notifyDiag(
                  '[Fatal] Existing background task ended (${update.status}): '
                  '${_taskStatusDetailForUser(update)}',
                );
                progress.addError(
                  DownloadException(
                    DownloadError.network(
                      'Existing download failed: ${_taskStatusDetailForUser(update)}',
                    ),
                  ),
                );
                progress.close();
        ...
        if (update is TaskProgressUpdate) {
          ...
          switch (update.status) {
            ...
            case TaskStatus.failed:
              debugPrint('🔴 DetailedSmartDownloader: TaskStatus.failed detected');
            ...
            case TaskStatus.canceled:
              _notifyDiag('[Fatal] TaskStatus.canceled — not retrying.');
              if (!progress.isClosed) {
                progress.addError(
            ...
            case TaskStatus.notFound:
              debugPrint('🔴 DetailedSmartDownloader: TaskStatus.notFound detected (404)');
      ...
      if (currentAttempt < maxRetries) {
        _notifyDiag(
          '[Retry] Caught exception (attempt $currentAttempt/$maxRetries): $e\n$st',
        );
        _notifyDiag(
          '[Retry] Waiting ${currentAttempt * 2}s before attempt ${currentAttempt + 1}/$maxRetries.',
        );
        ...
        _notifyDiag(
          '[Fatal] Exception after $maxRetries attempts — not retrying.',
        );
        if (!progress.isClosed) {
          progress.addError(
            DownloadException(
  ...
  static Future<bool> _handleFailedDownload({
    ...
    if (httpStatusCode != null) {
      ...
      if (httpStatusCode == 401) {
        ...
        _notifyDiag('[Fatal] HTTP 401 — not retrying.');
        if (!progress.isClosed) {
      ...
      if (httpStatusCode == 403) {
        _notifyDiag('[Fatal] HTTP 403 — not retrying.');
        if (!progress.isClosed) {
          progress.addError(
      ...
      if (httpStatusCode == 404) {
        _notifyDiag('[Fatal] HTTP 404 — not retrying.');
        if (!progress.isClosed) {
          progress.addError(
    ...
    try {
      ...
      if (canResume) {
        ...
        _notifyDiag('[Retry] Calling FileDownloader.resume(…) for task ${task.taskId}.');
      ...
      _notifyDiag('[Retry] resume() failed: $e\n$st');
      _lastResumeFailure = e;
    ...
    if (currentAttempt < maxRetries) {
      _notifyDiag(
        '[Retry] Full restart after ${currentAttempt * 2}s (next attempt '
      ...
      _notifyDiag(
        '[Fatal] Exhausted $maxRetries automatic attempts — not retrying.',
      );
      if (!progress.isClosed) {
        final resumeHint = _lastResumeFailure != null
            ? ' Last resume error: $_lastResumeFailure.'
            : '';
        final lastTask = taskFailure != null
            ? ' Last task failure: ${_taskStatusDetailForUser(taskFailure)}.'
            : '';
        progress.addError(
          DownloadException(
            DownloadError.unknown(
              'Download failed after $maxRetries attempts.$resumeHint$lastTask',
            ),
          ),
          StackTrace.current,
        );
  ...
  ///
  /// This is kept for backward compatibility but this downloader works with ANY URL.
  /// You don't need to check this before using [DetailedSmartDownloader].
  @Deprecated('DetailedSmartDownloader works with all URLs. No need to check anymore.')
